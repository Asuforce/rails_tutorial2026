...
class PasswordFormTest < PasswordResetForm
  ...
  test "reset with reight email but wrong token" do
...
class PasswordUpdateTest < PasswordResetForm
  ...
  test "update with valid password and confirmation" do
    @reset_user.remember
    assert_not_nil @reset_user.remember_digest
...
class ExpiredTokenTest < ExpiredToken
  ...
  test "should include the word 'expred' on the password-reset page" do
    follow_redirect!
